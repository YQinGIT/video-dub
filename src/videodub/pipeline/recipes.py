...
`full_dub` ends with `subtitle_sidecar`: once the dub is muxed, it writes the
translated lines as a `.srt` next to the video — sharing the video's stem — so a
media player loads them alongside the dubbed audio. It is a plain stage, not a
toggle; the dub always ships its matching subtitles.
...
RECIPES: dict[str, list[str]] = {
    "full_dub": [
        ...
        "tts", "timing", "mixing", "remux", "subtitle_sidecar",
    ],
    "translate_subtitles": [
